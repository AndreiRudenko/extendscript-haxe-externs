...
@:native("ProgressOperator") extern class ProgressOperator {
	...
	/* { b => inError, text => When  is , the problematic thumbnail., a => { text => operationStatus, href => /ProgressOperator/operationStatus } } */
	var errorTarget:Thumbnail; 

	/* { text => When  has a value, an array of the same length containing the new name strings to be assigned to the source  objects after they are transfered to the target., a => [{ text => sources, href => /ProgressOperator/sources },{ text => Thumbnail, href => /Thumbnail }] } */
	...
	/* { text => Implement a method that resolves a file-system conflict, as identified by the  and  values., a => [{ text => conflictType, href => /ProgressOperator/conflictType },{ text => conflictMessage, href => /ProgressOperator/conflictMessage }] } */
